<template>
    <!--子任务-->
    <div v-if="ready && isSubTask" class="subtask-wrapper">
        <li class="subtask-item">
            <div
                class="subtask-icon"
                :style="getFlowStatusStyle(taskDetail)">
                <TaskMenu
            ...
            <div class="subtask-name">
                <Input
                    v-if="subtaskNameEditing"
                    v-model="taskDetail.name"
                    ref="subtaskNameInput"
                    type="text"
                    :maxlength="255"
                    enterkeyhint="done"
                    @on-blur="finishEditSubtaskName"
                    @on-keydown="onNameKeydown"
                />
                <div
                    v-else
                    class="subtask-name-button"
                    @click="handleSubtaskNameClick"
                    @dblclick.stop="startEditSubtaskName">
                    {{ taskDetail.name }}
                </div>
            </div>
            <DatePicker
            ...
            <!-- 添加子任务按钮 -->
            <div v-if="!taskDetail.complete_at" class="subtask-add-child-btn" @click.stop="toggleSubTaskInput">
                <Icon :type="subTaskInputVisible ? 'md-remove' : 'md-add'" size="16"/>
            </div>
        </li>
        <!-- 子任务的子任务列表 -->
        <ul v-if="mySubList.length > 0 || subTaskInputVisible" class="nested-subtask-list">
            <TaskDetail
                v-for="(task, key) in mySubList"
            ...
            <!-- 添加子任务输入框 -->
            <li v-if="subTaskInputVisible" class="nested-subtask-add">
                <Input
                    v-model="subTaskInputName"
                    :ref="`subTaskInput_${taskDetail.id}`"
                    class="add-input"
                    :placeholder="$L('+ 输入子任务，回车添加子任务')"
                    :icon="subTaskInputLoading ? 'ios-loading' : ''"
                    :class="{loading: subTaskInputLoading}"
                    enterkeyhint="done"
                    @on-blur="onSubTaskInputBlur"
                    @on-keydown="onSubTaskInputKeydown"/>
            </li>
        </ul>
    </div>
    <!--主任务-->
    <div
        ...
        <div v-show="taskDetail.id > 0" class="task-info" v-resize-observer="scrollIntoInput">
            ...
            <Scrollbar ref="scroller" class="scroller" :touch-content-blur="false">
                ...
                <!-- 面包屑导航 -->
                <div v-if="taskBreadcrumbs.length > 1" class="task-breadcrumbs">
                    <span
                        v-for="(crumb, index) in taskBreadcrumbs"
                        :key="crumb.id"
                        class="breadcrumb-item">
                        <span
                            v-if="!crumb.isCurrent"
                            class="breadcrumb-link"
                            @click="navigateToTask(crumb.id)">
                            {{ crumb.name }}
                        </span>
                        <span v-else class="breadcrumb-current">
                            {{ crumb.name }}
                        </span>
                        <i v-if="index < taskBreadcrumbs.length - 1" class="taskfont breadcrumb-arrow">&#xe676;</i>
                    </span>
                </div>
                <div class="title">
                    <Input
                ...
                <Form class="items" label-position="left" label-width="auto" @submit.native.prevent>
                            ...
                            <li>
                                <DatePicker
                                    ...
                                    <div class="picker-time">
                                        ...
                                        <TaskPriority v-if="taskDetail.p_name" :backgroundColor="taskDetail.p_color" style="margin-left: 8px;">
                                            <span ref="priorityText" @click="onPriority">{{taskDetail.p_name}}</span>
                                        </TaskPriority>
                                    </div>
                                </DatePicker>
                            </li>
                    ...
                    <FormItem v-if="taskDetail.start_at && taskDetail.end_at">
                        <div class="item-label" slot="label">
                            <i class="taskfont">&#xe71d;</i>{{$L('里程碑')}}
                        </div>
                        <ul class="item-content milestones">
                            <li v-for="(milestone, idx) in milestoneList" :key="`milestone-${idx}`" class="milestone-item">
                                <Input
                                    v-model="milestone.name"
                                    :placeholder="$L('里程碑名称')"
                                    size="small"
                                    style="width: 150px; margin-right: 8px;"
                                    @on-blur="checkAndUpdateMilestone(idx)"/>
                                <DatePicker
                                    ...
                                    @on-change="date => handleDateChange(idx, date)"/>
                                <ColorPicker
                                    v-model="milestone.color"
                                    size="small"
                                    :recommend="false"
                                    @on-change="color => setMilestoneColor(idx, color)"/>
                                <Icon
                                    type="md-close-circle"
                                    size="18"
                                    style="cursor: pointer; color: #999; margin-left: 8px;"
                                    @click="removeMilestone(idx)"/>
                            </li>
                            <!-- Final 里程碑 -->
                            <li class="milestone-item milestone-final">
                                <Input
                                    v-model="finalMilestone.name"
                                    :placeholder="$L('Final 阶段名称')"
                                    size="small"
                                    style="width: 150px; margin-right: 8px;"
                                    @on-blur="updateFinalMilestone"/>
                                <Input
                                    :value="taskDetail.end_at ? $A.dayjs(taskDetail.end_at).format('YYYY-MM-DD') : '--'"
                                    disabled
                                    size="small"
                                    style="width: 130px; margin-right: 8px;"/>
                                <ColorPicker
                                    v-model="finalMilestone.color"
                                    size="small"
                                    :recommend="false"
                                    @on-change="updateFinalMilestone"/>
                                <span style="display: inline-block; width: 18px; margin-left: 8px;"></span>
                            </li>
                            <li class="milestone-actions">
                                <Button size="small" icon="md-add" @click="addMilestone">{{$L('添加里程碑')}}</Button>
                                <Button size="small" @click="showBatchCreateModal">{{$L('批量创建')}}</Button>
                            </li>
                        </ul>
                    </FormItem>
                        ...
                        <ul class="item-content subtask-add">
                            <li>
                                ...
                                <Button
                                    ...
                                    @click="showBatchCreateSubtaskModal">{{$L('批量创建')}}</Button>
                            </li>
                        </ul>
                    </FormItem>
        ...
        <Modal
            ...
            :mask-closable="false"
            width="500">
            <div class="batch-milestone-form">
                <div class="form-tip">
                    <Icon type="ios-information-circle" />
                    {{$L('里程碑将任务分为多个阶段，最后一段自动显示为"Final"（跟随任务结束日期）')}}
                </div>
                <div class="form-row">
                    <label>{{$L('阶段数量')}}:</label>
                    <InputNumber v-model="batchCount" :min="2" :max="10" style="width: 120px;"/>
                    <span style="margin-left: 8px; color: #999; font-size: 12px;">
                        {{$L('将创建')}} {{batchCount - 1}} {{$L('个里程碑')}}
                    </span>
                </div>
                <div class="form-row">
                    <label>{{$L('预览')}}:</label>
                    <div class="milestone-preview">
                        <div v-for="(item, idx) in batchPreview" :key="idx" class="preview-item">
                            <Input v-model="item.name" size="small" style="width: 120px; margin-right: 8px;"/>
                            <DatePicker
                                v-model="item.date"
                                type="date"
                                format="yyyy-MM-dd"
                            ...
                            <div
                                ...
                                @click="showColorPicker(idx)"/>
                        </div>
                    </div>
                </div>
                <div v-if="colorPickerIndex !== null" class="color-picker-panel">
                    <div class="color-picker-title">{{$L('选择颜色')}}:</div>
                    <div class="color-options">
                        <div
                            v-for="color in milestoneColors"
                            :key="color"
                            class="color-option-large"
                            :class="{ 'active': batchPreview[colorPickerIndex].color === color }"
                            :style="{ backgroundColor: color }"
                            @click="setBatchColor(colorPickerIndex, color)"/>
                    </div>
                </div>
            </div>
            <div slot="footer">
                <Button @click="batchCreateShow=false">{{$L('取消')}}</Button>
                <Button type="primary" @click="confirmBatchCreate">{{$L('确定')}}</Button>
            </div>
        </Modal>
        <!--批量创建子任务-->
        <Modal
            v-model="batchCreateSubtaskShow"
            :title="$L('批量创建子任务')"
            :mask-closable="false"
            width="500">
            <div class="batch-milestone-form">
                <div class="form-tip">
                    <Icon type="ios-information-circle" />
                    {{$L('快速创建多个子任务，可以自定义每个子任务的名称')}}
                </div>
                <div class="form-row">
                    <label>{{$L('子任务数量')}}:</label>
                    <InputNumber v-model="batchSubtaskCount" :min="2" :max="20" style="width: 120px;"/>
                </div>
                <div class="form-row">
                    <label>{{$L('预览')}}:</label>
                    <div class="milestone-preview">
                        <div v-for="(item, idx) in batchSubtaskPreview" :key="idx" class="preview-item">
                            <span style="margin-right: 8px; color: #999; width: 30px; display: inline-block;">{{idx + 1}}.</span>
                            <Input v-model="item.name" size="small" style="flex: 1;"/>
                        </div>
                    </div>
                </div>
            </div>
            <div slot="footer">
                <Button @click="batchCreateSubtaskShow=false">{{$L('取消')}}</Button>
                <Button type="primary" @click="confirmBatchCreateSubtask">{{$L('确定')}}</Button>
            </div>
        </Modal>
        <!--标签添加-->
...
export default {
    ...
    data() {
        return {
            ...
            // 子任务的添加子任务状态
            subTaskInputVisible: false,
            subTaskInputName: "",
            subTaskInputLoading: false,

            milestoneList: [],
            milestoneColors: ['#4A90E2', '#F5A623', '#FF6B6B', '#50E3C2', '#9013FE', '#7ED321', '#BD10E0', '#4A4A4A'],
            finalMilestone: {
                name: 'Final',
                color: '#52c41a'
            },
            ...
            batchCreateSubtaskShow: false,
            batchSubtaskCount: 5,
            batchSubtaskPreview: [],

            subtaskNameEditing: false,
            subtaskNameClickTimer: null,
    ...
    computed: {
        ...
        taskBreadcrumbs() {
            const breadcrumbs = [];
            let currentTask = this.taskDetail;

            // 向上遍历父任务
            while (currentTask && currentTask.parent_id) {
                const parentTask = this.cacheTasks.find(({id}) => id == currentTask.parent_id);
                if (parentTask) {
                    breadcrumbs.unshift({
                        id: parentTask.id,
        ...
        isSubTask({taskDetail, mainEndAt}) {
            // 只有在父任务的子任务列表中显示时才使用简化视图
            // 单独打开子任务详情时（mainEndAt 为 null），显示完整视图
            return taskDetail.parent_id > 0 && mainEndAt !== null
        },
    ...
    watch: {
        openTask: {
            handler(data) {
                const oldTaskId = this.taskDetail?.id;
                ...
                // 初始化里程碑列表
                if (oldTaskId !== data.id) {
                    // 切换任务时，完全重新初始化
                    this.milestoneList = Array.isArray(data.milestones) ? $A.cloneJSON(data.milestones) : [];
                    // 初始化 Final 里程碑
                    this.finalMilestone = {
                        name: data.final_milestone_name || 'Final',
                        color: data.final_milestone_color || '#52c41a'
                    };
                } else {
                    // 同一任务更新时，合并服务器数据和本地正在编辑的数据
                    const serverMilestones = Array.isArray(data.milestones) ? data.milestones : [];
                    const editingMilestones = this.milestoneList.filter(m => !m.name || !m.name.trim());
                    this.milestoneList = [...serverMilestones, ...editingMilestones];
                    // 同步更新 Final 里程碑
                    this.finalMilestone = {
                        name: data.final_milestone_name || 'Final',
                        color: data.final_milestone_color || '#52c41a'
        ...
        batchCount(val) {
            if (this.batchCreateShow && val >= 2 && val <= 10) {
                this.generateBatchPreview();
            }
        },
        batchSubtaskCount(val) {
            if (this.batchCreateSubtaskShow && val >= 2 && val <= 20) {
                this.generateBatchSubtaskPreview();
            }
        },
        getOwner: {
            handler(arr) {
    ...
    methods: {
        navigateToTask(taskId) {
            // 跳转到指定任务的详情页
            this.$store.dispatch("openTask", {id: taskId});
        },

        handleSubtaskNameClick() {
            // 使用延迟区分单击和双击
            if (this.subtaskNameClickTimer) {
                // 如果已有定时器，说明是双击，清除定时器不执行单击
                clearTimeout(this.subtaskNameClickTimer);
                this.subtaskNameClickTimer = null;
                return;
            }

            // 设置延迟执行单击操作
            this.subtaskNameClickTimer = setTimeout(() => {
                this.navigateToTask(this.taskDetail.id);
                this.subtaskNameClickTimer = null;
            }, 150);
        },

        startEditSubtaskName() {
            // 双击开始编辑子任务名称
            // 清除单击定时器
            if (this.subtaskNameClickTimer) {
                clearTimeout(this.subtaskNameClickTimer);
            ...
            this.subtaskNameEditing = true;
            this.$nextTick(() => {
                this.$refs.subtaskNameInput?.focus();
            });
        },

        finishEditSubtaskName() {
            // 失去焦点时保存并退出编辑模式
            this.subtaskNameEditing = false;
            this.updateBlur('name');
        },

        getFlowStatusStyle(task) {
            // 根据流程状态返回圆圈的背景色
            if (!task.flow_item_status) {
                return {
                    backgroundColor: '#d9d9d9'  // 默认灰色
                };
            }

            const statusColors = {
                'start': '#ff4d4f',      // 待处理 - 红色
                'progress': '#ff9900',   // 进行中 - 橙色
                'test': '#1890ff',       // 待测试 - 蓝色
                'end': '#52c41a'         // 已完成 - 绿色
            };

            const color = task.flow_item_color || statusColors[task.flow_item_status] || '#d9d9d9';
        ...
        // 子任务的添加子任务方法
        toggleSubTaskInput() {
            this.subTaskInputVisible = !this.subTaskInputVisible;
            if (this.subTaskInputVisible) {
                this.$nextTick(() => {
                    const ref = this.$refs[`subTaskInput_${this.taskDetail.id}`];
                    if (ref) {
                        ref.focus();
                    }
                });
            } else {
                this.subTaskInputName = "";
            }
        },

        onSubTaskInputBlur() {
            if (this.subTaskInputName === '') {
                this.subTaskInputVisible = false;
            }
        },

        onSubTaskInputKeydown(e) {
            if (e.keyCode === 13) {
                if (e.shiftKey || this.subTaskInputLoading) {
                    return;
                }
                e.preventDefault();
                this.onAddSubTaskChild();
            }
        },
        ...
        onAddSubTaskChild() {
            ...
            this.subTaskInputLoading = true;
            this.$store.dispatch("taskAddSub", {
                task_id: this.taskDetail.id,  // 当前子任务作为父任务
                name: this.subTaskInputName,
            }).then(({msg}) => {
                $A.messageSuccess(msg);
                this.subTaskInputName = "";
                // 保持输入框打开，方便连续添加
                ...
                this.subTaskInputLoading = false;
            });
        },

        // 添加里程碑
        addMilestone() {
            if (!this.taskDetail.start_at || !this.taskDetail.end_at) {
                $A.messageWarning('请先设置任务的开始和结束时间');
                return;
            }

            const taskStart = $A.dayjs(this.taskDetail.start_at);
            const taskEnd = $A.dayjs(this.taskDetail.end_at);
            const existingMilestones = this.milestoneList.filter(m => m.name && m.name.trim());

            if (existingMilestones.length === 0) {
                // 第一个里程碑：在中间点创建
                const totalDays = taskEnd.diff(taskStart, 'day');
                const midDate = taskStart.add(Math.floor(totalDays / 2), 'day');

                this.milestoneList.push({
                    name: 'V001',
                    date: midDate.format('YYYY-MM-DD'),
                    color: this.milestoneColors[0]
                });
                ...
                this.milestoneList.push({
                    name: '',
                    date: $A.dayjs().format('YYYY-MM-DD'),
                    color: this.milestoneColors[this.milestoneList.length % this.milestoneColors.length]
                });
            }
        },
        ...
        // 设置里程碑颜色
        setMilestoneColor(index, color) {
            // ColorPicker 可能返回 hex 字符串或对象，统一处理为字符串
            const colorValue = typeof color === 'string' ? color : (color?.hex || color);
            this.$set(this.milestoneList[index], 'color', colorValue);
            this.checkAndUpdateMilestone(index);
        },
        ...
        generateBatchPreview() {
            const start = $A.dayjs(this.taskDetail.start_at);
            const end = $A.dayjs(this.taskDetail.end_at);
            const totalDays = end.diff(start, 'day');
            const segmentDays = Math.floor(totalDays / this.batchCount);

            this.batchPreview = [];
            // 只创建 batchCount-1 个里程碑，最后一段自动为Final
            for (let i = 0; i < this.batchCount - 1; i++) {
                const milestoneDate = start.add((i + 1) * segmentDays, 'day');

                this.batchPreview.push({
                    name: `V${String(i + 1).padStart(3, '0')}`,
                    date: milestoneDate.toDate(),
                    color: this.milestoneColors[i % this.milestoneColors.length]
                });
            }
        },
        ...
        showColorPicker(index) {
            this.colorPickerIndex = this.colorPickerIndex === index ? null : index;
        },

        // 设置批量创建的颜色
        setBatchColor(index, color) {
            this.$set(this.batchPreview[index], 'color', color);
            this.colorPickerIndex = null;
        },

        // 更新 Final 里程碑
        updateFinalMilestone() {
            // 保存 Final 里程碑的名称和颜色，同时更新任务条颜色
            const colorValue = typeof this.finalMilestone.color === 'string'
                ? this.finalMilestone.color
                : (this.finalMilestone.color?.hex || this.finalMilestone.color);

            console.log('[updateFinalMilestone] 准备发送数据:', {
                task_id: this.taskDetail.id,
                final_milestone_name: this.finalMilestone.name,
                final_milestone_color: colorValue,
                color: colorValue
            });
            ...
            this.$store.dispatch("taskUpdate", {
                ...
                final_milestone_name: this.finalMilestone.name,
                final_milestone_color: colorValue,
                color: colorValue  // 同时更新任务条颜色
            }).then((result) => {
                console.log('[updateFinalMilestone] 更新成功，返回数据:', result);
                // taskUpdate 已经通过 saveTask 更新了缓存，不需要手动更新
            }).catch(({msg}) => {
                console.error('[updateFinalMilestone] 更新失败:', msg);
                $A.modalError(msg);
            });
        },
        ...
        // 确认批量创建
        confirmBatchCreate() {
            const newMilestones = this.batchPreview.map(item => ({
                name: item.name,
                date: $A.dayjs(item.date).format('YYYY-MM-DD'),
                color: item.color
            }));

            this.milestoneList = [...this.milestoneList, ...newMilestones];
            this.updateMilestones();
            this.batchCreateShow = false;
        },

        // 显示批量创建子任务模态框
        showBatchCreateSubtaskModal() {
            this.batchSubtaskCount = 5;
            this.generateBatchSubtaskPreview();
            this.batchCreateSubtaskShow = true;
        },

        // 生成批量子任务预览
        generateBatchSubtaskPreview() {
            this.batchSubtaskPreview = [];
            for (let i = 0; i < this.batchSubtaskCount; i++) {
                this.batchSubtaskPreview.push({
                    name: `${this.$L('子任务')} ${i + 1}`
                });
            }
        },
        ...
        // 确认批量创建子任务
        confirmBatchCreateSubtask() {
            const validSubtasks = this.batchSubtaskPreview.filter(item => item.name && item.name.trim());

            if (validSubtasks.length === 0) {
                $A.messageWarning(this.$L('请至少填写一个子任务名称'));
                return;
            }

            // 批量创建子任务 - 直接复用 taskAddSub，和单个创建逻辑一致
            let createdCount = 0;
            validSubtasks.forEach((item) => {
                this.$store.dispatch("taskAddSub", {
                    ...
                    createdCount++;
                    if (createdCount === validSubtasks.length) {
                        $A.messageSuccess(this.$L('批量创建成功'));
                    }
                }).catch(({msg}) => {
                    $A.messageError(msg || this.$L('创建失败'));
                });
            });

            this.batchCreateSubtaskShow = false;
        },


        // 检查并更新里程碑（只有名称和日期都有值时才保存）
        checkAndUpdateMilestone(index) {
            const milestone = this.milestoneList[index];
            if (milestone && milestone.name && milestone.name.trim() && milestone.date) {
                this.updateMilestones();
            }
        },

        // 处理日期变化（DatePicker 返回 Date 对象，需要转换为字符串）
        handleDateChange(index, date) {
            if (date) {
                // 使用 $set 确保响应式更新
                this.$set(this.milestoneList[index], 'date', $A.dayjs(date).format('YYYY-MM-DD'));
                // 检查是否需要保存（名称和日期都有值）
                const milestone = this.milestoneList[index];
        ...
        updateMilestones() {
            // 过滤掉空名称的里程碑
            const validMilestones = this.milestoneList.filter(m => m.name && m.name.trim()).map(m => ({
                name: m.name.trim(),
                date: typeof m.date === 'string' ? m.date : $A.dayjs(m.date).format('YYYY-MM-DD'),
                color: m.color || this.milestoneColors[0]
            }));

            console.log('[updateMilestones] 准备发送数据:', {
                task_id: this.taskDetail.id,
            ...
            this.$store.dispatch("taskUpdate", {
                ...
                console.log('[updateMilestones] 更新成功，返回数据:', result);
                // taskUpdate 已经通过 saveTask 更新了缓存，不需要手动更新
            }).catch(({msg}) => {
                console.error('[updateMilestones] 更新失败:', msg);
                $A.modalError(msg);
            });
        },
...
// ColorPicker 弹出层需要最高的 z-index，确保不被子任务遮挡
::v-deep .ivu-color-picker-dropdown {
